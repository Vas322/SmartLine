...
class WelcomeSettingsWebTests(TestCase):
    ...
    def test_settings_redirects_edit_welcome_get_to_welcome_page(self):
        self._login(self.staff)
        response = self.client.get(reverse("settings") + "?edit_welcome=1")
        self.assertRedirects(
            response,
            reverse("settings_welcome") + "?edit=1",
            status_code=302,
            target_status_code=200,
        )

    def test_settings_redirects_welcome_post_to_welcome_page(self):
        self._login(self.staff)
        response = self.client.post(
            reverse("settings"),
            {"save_welcome": "1", "welcome_text": "Добро пожаловать в клан!"},
        )
        self.assertRedirects(response, reverse("settings_welcome"))

    def test_welcome_page_renders_welcome_text(self):
        settings = welcome_service.get_welcome_settings()
        settings.welcome_text = "Добро пожаловать в клан!"
        settings.save()

        self._login(self.staff)
        response = self.client.get(reverse("settings_welcome"))
    ...
    def test_welcome_page_shows_disabled_hint_when_text_empty(self):
        self._login(self.staff)
        response = self.client.get(reverse("settings_welcome"))
        content = response.content.decode()
        self.assertIn("Приветствия выключены (текст пуст)", content)

    def test_welcome_page_opens_edit_form_with_edit_param(self):
        settings = welcome_service.get_welcome_settings()
        settings.welcome_text = "Добро пожаловать в клан!"
        settings.save()

        self._login(self.staff)
        response = self.client.get(reverse("settings_welcome") + "?edit=1")
        content = response.content.decode()
        self.assertIn('name="welcome_text"', content)
        self.assertIn('name="save_welcome" value="1"', content)
        self.assertIn("Отмена", content)

    def test_welcome_page_save_welcome_text(self):
        self._login(self.staff)
        response = self.client.post(
            reverse("settings_welcome"),
            {"save_welcome": "1", "welcome_text": "Добро пожаловать в клан!"},
        )
        self.assertRedirects(response, reverse("settings_welcome"))
        settings = welcome_service.get_welcome_settings()
    ...
    def test_welcome_page_reset_welcome_block(self):
        settings = welcome_service.get_welcome_settings()
    ...
    def test_welcome_page_shows_blocked_message_when_blocked(self):
        settings = welcome_service.get_welcome_settings()
        settings.block_until = timezone.now() + timedelta(minutes=30)
        settings.save()
    ...
    def test_settings_page_does_not_contain_welcome_section(self):
        self._login(self.staff)
        response = self.client.get(reverse("settings"))
        self.assertEqual(response.status_code, 200)
        content = response.content.decode()
        self.assertNotIn('name="welcome_text"', content)
        self.assertNotIn('name="save_welcome"', content)
        self.assertNotIn('name="reset_welcome_block"', content)
        self.assertNotIn("Приветствия выключены (текст пуст)", content)
        self.assertNotIn("?edit_welcome=1", content)

    def test_settings_welcome_tile_links_to_welcome_page(self):
        self._login(self.staff)
        response = self.client.get(reverse("settings"))
        content = response.content.decode()
        self.assertIn(f'href="{reverse("settings_welcome")}"', content)

    def test_non_staff_cannot_access_settings_welcome(self):
